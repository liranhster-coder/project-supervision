...
export async function POST(request: Request) {
  ...
  if (!visit || !project) return NextResponse.json({ error: 'Not found' }, { status: 404 })
        ...
        new Table({
          ...
          rows: [
            new TableRow({ children: [
              new TableCell({ children: [new Paragraph({ text: 'פרויקט:', run: { bold: true } })] }),
              new TableCell({ children: [new Paragraph(project.name)] }),
              new TableCell({ children: [new Paragraph({ text: 'תאריך:', run: { bold: true } })] }),
              new TableCell({ children: [new Paragraph(visitDate)] }),
            ]}),
            ...(project.address ? [new TableRow({ children: [
              new TableCell({ children: [new Paragraph({ text: 'כתובת:', run: { bold: true } })] }),
